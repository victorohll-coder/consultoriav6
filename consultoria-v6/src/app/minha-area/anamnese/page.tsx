...
export default function AnamnesePacientePage() {
  ...
  if (loading) return <div className="text-[#475569] text-sm p-6">Carregando...</div>;
  ...
  if (anamnese) {
    return (
      <div>
        ...
        <div className="bg-[#059669]/10 border border-[#059669]/30 rounded-xl p-4 mb-6">
          <p className="text-[#059669] text-sm font-semibold">✓ Anamnese preenchida</p>
        ...
        {BLOCOS_ANAMNESE.map((bloco) => (
          <div key={bloco} className="mb-6">
            <h2 className="text-xs font-bold text-[#2563eb] uppercase tracking-wider mb-3 flex items-center gap-2">
            ...
            <div className="flex flex-col gap-2">
              {CAMPOS_ANAMNESE.filter((c) => c.bloco === bloco).map((campo) => {
                ...
                return (
                  <div key={campo.id} className="bg-white border border-[#e0eaf5] rounded-xl p-4">
                    <p className="text-[11px] font-semibold text-[#475569] mb-1">
                      {campo.label}
                    </p>
                    <p className="text-sm text-[#0f172a] whitespace-pre-wrap">
                      {campo.tipo === "escala" && val ? `${val}/10` : val || <span className="text-[#9ca3af] italic">Não informado</span>}
                    </p>
                  </div>
                );
              })}
    ...
    <div>
      ...
      {success && (
        <div className="bg-[#059669]/10 border border-[#059669]/30 rounded-xl p-4 mb-4">
          <p className="text-[#059669] text-sm font-semibold">✓ Anamnese salva com sucesso!</p>
          <p className="text-[#475569] text-xs mt-1">Obrigado por responder. Seu nutricionista já pode visualizar.</p>
        </div>
      )}
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-semibold text-[#2563eb]">{blocoAtual}</span>
          <span className="text-[10px] text-[#9ca3af]">{currentBloco + 1} de {BLOCOS_ANAMNESE.length}</span>
        </div>
        ...
        {camposBlocoAtual.map((campo) => (
          <div key={campo.id} className="bg-white border border-[#e0eaf5] rounded-xl p-4">
            <label className="block text-sm font-semibold text-[#0f172a] mb-2">
              {campo.label}
              {campo.id === "objetivo" && <span className="text-[#dc2626] ml-1">*</span>}
            </label>
            ...
            {campo.tipo === "textarea" && (
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 bg-[#f8fafc] border border-[#e0eaf5] rounded-lg text-[#0f172a] text-sm focus:border-[#2563eb] focus:outline-none resize-none transition-colors"
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm border transition-all ${
                      respostas[campo.id] === op
                        ...
                        : "bg-[#f8fafc] border-[#e0eaf5] text-[#475569] hover:border-[#2563eb]/50"
                    }`}
                  >
                    {op}
                  </button>
            ...
            {campo.tipo === "escala" && (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex justify-between text-[10px] text-[#9ca3af]">
                  <span>1 — Pouco</span>
                  <span className="text-[#2563eb] font-bold text-sm">{respostas[campo.id] || 5}</span>
                  <span>10 — Máximo</span>
                </div>
              </div>
            )}
      ...
      <div className="flex gap-3">
        {!isFirstBloco && (
          <button
            ...
            className="flex-1 bg-[#f1f5f9] hover:bg-[#f1f5f9] text-[#0f172a] font-semibold py-3 rounded-xl text-sm transition-colors border border-[#e0eaf5]"
          >
            ← Voltar
          </button>
        )}
        ...
        {!isLastBloco ? (
          <button
            onClick={() => setCurrentBloco(currentBloco + 1)}
            className="flex-1 bg-[#2563eb] hover:bg-[#172e8a] text-white font-semibold py-3 rounded-xl text-sm transition-colors"
          >
            Próximo →
          </button>
        ) : (
          <button
            ...
            disabled={submitting}
            className="flex-1 bg-[#059669] hover:bg-[#059669]/80 text-white font-semibold py-3 rounded-xl text-sm transition-colors disabled:opacity-50"
          >
